...
// โครงสร้างโพสต์สำหรับแสดงผลใน UI
interface Post {
  id: string;
  content: string;
  created_at: string;
...
export default function Page() {
  ...
  const [avatarPreview, setAvatarPreview] = useState<string | null>(null); // Preview Avatar
  ...
  const [coverPreview, setCoverPreview] = useState<string | null>(null); // Preview Cover
  const [coverPublicUrl, setCoverPublicUrl] = useState<string | null>(null); // Public URL Cover ปัจจุบัน
  const [message, setMessage] = useState<{ text: string; type: "success" | "error" } | null>(null); // ข้อความแจ้งเตือน
  ...
  const [groupsMap, setGroupsMap] = useState<Record<string, { id: string; name: string }>>({});
  ...
  const fetchPosts = async (userId: string) => {
    ...
    if (!error && data) {
      ...
      // ดึงชื่อกลุ่มที่เกี่ยวข้องทั้งหมดมาเก็บไว้ (Batch Fetching)
      ...
      // แปลงข้อมูลโพสต์ให้อยู่ในรูปแบบที่พร้อมแสดงผล
  ...
  const handleProfileLikeToggle = async (postId: string, isLiked: boolean) => {
    ...
    // Optimistic Update: อัปเดตหน้าจอทันทีก่อนยิง API
    setPosts((prevPosts) =>
      prevPosts.map((p) =>
        p.id === postId
          ? {
    ...
    try {
      ...
      // หาก error ให้ Rollback UI (ย้อนค่ากลับ)
      setPosts((prevPosts) =>
        prevPosts.map((p) =>
          p.id === postId
            ? {
  ...
  const handleUpdateProfile = async (e: FormEvent) => {
    ...
    try {
      ...
      if (coverFile) {
        ...
        const { data, error } = await supabase.storage.from("avatars").upload(path, coverFile);
        if (error) throw error;
        newCoverPath = data.path;

        const { data: urlData } = supabase.storage.from("avatars").getPublicUrl(newCoverPath);
      ...
      // เนื่องจากมีการอัปเดต Public URL แล้ว จึงไม่ต้องเรียก fetchProfile ซ้ำ
      // แต่ควรตั้งค่า Profile State ด้วยข้อมูลใหม่
      setProfile((prev) => ({ 
          ...prev!, 
          username: usernameEdit, 
          avatar_url: newAvatarPath, 
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 pb-20">
      ...
      {showImageModal && (
        ...
        >
          <div className="relative max-w-4xl max-h-[90vh] w-full h-full flex items-center justify-center">
            ...
            <button 
              onClick={() => setShowImageModal(false)}
              className="absolute top-4 right-4 text-white text-3xl font-bold p-2 rounded-full hover:bg-white hover:bg-opacity-20 transition"
              aria-label="ปิด"
            >
              &times;
            </button>
      ...
      {/* ส่วน Header (Cover + Avatar + ข้อมูลส่วนตัว) */}
      <div className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-5xl mx-auto">
          ...
          <div 
            ...
            onClick={() => displayCoverUrl !== DEFAULT_COVER && handleImageClick(displayCoverUrl)}
          >
            <Image
              src={displayCoverUrl}
              alt="Cover"
      ...
      {/* ส่วนฟอร์มแก้ไขโปรไฟล์ (แสดงเมื่อกดปุ่มแก้ไข) */}
      {isEditing && (
      ...
      {/* ✅ Modal คอมเมนต์ (แสดงที่ Root Level) */}
