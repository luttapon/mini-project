...
interface ProfileCommentModalProps {
    postId: string;
    userId: string; // ID ของ User ที่กำลังจะคอมเมนต์ (User ที่ล็อกอินอยู่)
    onClose: () => void;
    updateCount: (postId: string) => void; // Callback เพื่อบอกหน้าหลักให้อัปเดต Comment Count
}

const COMMENTS_LIMIT = 3; 

// Helper function: Get public URL for user avatar
const getAvatarPublicUrl = (path: string | null | undefined) => {
    if (!path) return "https://via.placeholder.com/32"; 
    if (path.startsWith("http://") || path.startsWith("https://")) return path; 
    const { data } = supabase.storage.from("avatars").getPublicUrl(path);
    return data.publicUrl || "https://via.placeholder.com/32";
};
...
export default function ProfileCommentModal({ postId, userId, onClose, updateCount }: ProfileCommentModalProps) {
    ...
    const handleCommentSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            // Update parent state (increment count)
            updateCount(postId); 
