...
export default function DashboardCommentModal({ postId, userId, onClose, updateCount }: DashboardCommentModalProps) {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      ...
      if (data) {
        // แปลงข้อมูลให้ปลอดภัย (Handle Null/Undefined) เพื่อป้องกัน Error
        const safeComments: CommentWithUser[] = data.map(c => ({
          ...c,
          // หากข้อมูล user เป็น null ให้ใส่ค่า default
  ...
  const handleCommentSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const { data: insertedData, error: insertError } = await supabase
        ...
        .select("*, user:user_id(id, username, avatar_url)") // select เพื่อให้ได้ข้อมูล user มาอัปเดต UI
        .single();
      ...
      // ⚠️ NOTE: alert() is used here. For production deployment, use a custom modal UI.
